...
const safeLookup = ((
  ...
  lookup(
    ...
    (error, addresses) => {
      ...
      const allowed = allowPrivateOutboundTargets()
        ? addresses
...
const secureDispatcher = new Agent({ connect: { lookup: safeLookup } });

export const secureFetch: typeof fetch = (async (input, init) =>
  (await undiciFetch(
    input as string | URL,
    {
      ...init,
